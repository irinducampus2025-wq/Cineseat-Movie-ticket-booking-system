...
void actionFind(void) {
    ...
    int mode = getInBetween("Pick: ", 1,2);
    ...
    if(mode == 1) {
                    ...
                    for (int c = 0; c < SEAT_COLS; c++) {
                            ...
                            if (tk->taken && strstr(tk->bookedBy, query)) {
                                printf(CLR_OK "%s - %s (%s) seat %c%d - Rs. %.2f\n"CLR_RESET,
                                       tk->bookdBy, films[f].title,films[f].shows[s].time,
      ...
      if (!hits) printf(CLR_ERR " NOthing matched \"%s\".\n" CLR_RESET,query);
        ...
        int r,c;
        if (!parseSeatCode(code, &r, &c)) { printf(CLR_ERR " Not a valid seat.\n"CLR_RESET); return; }
        Ticker *tk = &films[f].shows[s].seats[r][c];
        if (!tk->taken) { printf( printf(CLR_ERR " %c%d is still free.\n" CLR_ERR " Not a valid seat.\n"1);return; }
        printf(CLR_OK "%c%d -> %s, paid Rs. %.2f\n" CLR_RESET, 'A' + r, c + 1, tk-> bookedBy, tk->amountPaid);
